...
internal struct OffsetPolygonCalculator: OffsetGeometryCalculator {
    ...
    func geometry(for translation: CGPoint, from geometry: Polygon) -> Polygon? {
        ...
        outerRing = targetPoints.map {$0.coordinates.coordinate2D}
        ...
        if !geometry.innerRings.isEmpty {
            ...
            for ring in geometry.innerRings {
                ...
                innerRing = targetPoints.map {$0.coordinates.coordinate2D}
                guard let innerRing = innerRing else { return nil }
                innerRings.append(Ring.init(coordinates: innerRing.map { LocationCoordinate2D(latitude: $0.longitude, longitude: $0.latitude)}))

            }
            return Polygon(outerRing: .init(coordinates: outerRing.map { LocationCoordinate2D(latitude: $0.latitude, longitude: $0.longitude)}), innerRings: innerRings)
        }
        return Polygon(outerRing: .init(coordinates: outerRing.map { LocationCoordinate2D(latitude: $0.latitude, longitude: $0.longitude)}))
    }
}
